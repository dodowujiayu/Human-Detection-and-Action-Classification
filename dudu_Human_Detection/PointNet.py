...
class HAR_model(nn.Module):
    def __init__(self, output_dim=5, frame_num = 60,hidden_size=16):
        ...
        self.lstm_net = nn.LSTM(1024, hidden_size,num_layers=1, dropout=0,bidirectional=True)
        self.dense = nn.Sequential(
            nn.Linear(frame_num*2*hidden_size,output_dim),
            # nn.Softmax(),
            )
